...
public class HeadingListView extends RelativeLayout implements WearableListView.OnScrollListener {
    ...
    private boolean scrollHeaderEnabled = false;
    ...
    @Override public void onAbsoluteScrollChange(final int i) {
        if (isScrollHeaderEnabled()) {
            // default behaviour in my setup
        }
        else {
    ...
    public boolean isScrollHeaderEnabled () {
        return scrollHeaderEnabled;
    }
    public void setScrollHeaderEnabled (boolean scrollHeader) {
        this.scrollHeaderEnabled = scrollHeader;
    }
}
