...
public class BearingSectorizer{
    ...
    private static String TAG = "BearingSectorizer";
    private boolean extremeDebugging = false;
    ...
    /* N, W, S, E */
    public static int QUADRANTS = 4;
    /** n, nw, w etc */
    public static int  OCTANTS = 8;
    /** n, nnw, nw, wnw, w etc */
      ...
      //  Log.d("SectoredOrientation","Got " + orientation + " last was " + mLastKnownSectorDegrees);
        ...
        if (mHasLastKnownSector) {
            ...
            if (delta < (mHalfSectorDegrees + mSectorHysteresisInDegrees)) {
                if (extremeDebugging) {
                    Log.d(TAG, "returning unchanged: " + mLastKnownSectorDegrees + "(delta:" + delta + " is less than " + (mHalfSectorDegrees + mSectorHysteresisInDegrees));
                }
        ...
        if (orientation != input) {
            Log.d(TAG, "clamped to :" + orientation + " from:" + input);
        }
        if (mLastKnownSectorDegrees != orientation) {
           Log.d(TAG, "returning changed: " + orientation);
        }
        else {
